...
__email__ = 'xf010101xf@gamil.com'
...
async def post_packet(url, headers, data_raw):
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, data=data_raw) as response:
            return await response.text()
            
# Example usage of send_packet() function
async def Replayers(command, url, headers, data_raw):
    if command == "GET":
        response = await get_request(url, headers)
    elif command == "POST":
        response = await post_packet(url, headers, data_raw)
    # print(response)
    # print("响应头部：  ",str(response.headers()))
...
chushi = "你好，我是一个语言模型AI，没有实体名字，你可以随意称呼我。"

def getgptinfo(input_text, assistant_content):
    try:
        # 构建请求数据
        data = {"messages": [
            {"role": "system", "content": "Professional code developers, Vulnerability Analysts"},
        ...
        gptinfo = output_text["choices"][0]["message"]["content"]
        # print(output_text)
        # print('问题是:\n ',input_text)
        # print("\n")
        # print("GPT3.5 针对数据包的检查建议")
        # print(gptinfo.strip())
        print("***************************************************************\n GPT3.5 针对数据包的检查建议:\n")
        print(gptinfo.strip())
        # count += 1
...
class Response(HttpTransfer):
    ...
    def _text(self):
        ...
        if self.get_header('Content-Type') and ('text' or 'javascript') in self.get_header('Content-Type'):
            ...
            if self.decoding:
                try:
                    self._body_str = body_data.decode(self.decoding, 'ignore')  # 请求体
                except Exception as e:
    ...
    def get_body_str(self,decoding=None):
        if decoding:
            try:
                return self.get_body_data().decode(decoding, 'ignore')
            except Exception as e:
                return None
...
class LewisInterceptor(ReqIntercept, RspIntercept):

    def deal_request(self, request):
        if "443" in str(request.port):
            url =  "https://"+request.hostname+":"+str(request.port)
            # logging.info('Request URL: %s',url,)
        else:
            url =  "http://"+request.hostname+":"+str(request.port)
            # logging.info('Request URL: %s',url,)
        headers = request.get_headers()
        # logging.info('Request Headers: %s', headers)
        data_raw = request.get_body_data().decode('utf-8', 'ignore')
        # logging.info('Request Body: %s', data_raw)
        command = request.command
        # logging.info('Request command: %s',command)
        # logging.info('Request:\n%s', request.to_data().decode('utf-8'))
        # asyncio.ensure_future(self.replay_request(request))
        # asyncio.run(Replayers(command, url, headers, data_raw))
        
        return request
   
    def deal_response(self,response):
        # logging.info('response:\n%s', response.to_data().decode('utf-8'))
        tmp = response.request.to_data().decode('utf-8', 'ignore')
        tmp2 = response.to_data().decode('utf-8', 'ignore')
        input_text4 = "{} \n {} \n {}".format(tmp, tmp2, "分析这个数据包，获取以下信息：1.网站和服务器相关信 2.可能存在哪些漏洞 以及 可以怎么样修改数据包")
        # print(input_text4)
        # print("request:\n{}".format(tmp))
        # print("response:\n{}".format(tmp2))
        # asyncio.run(send2gpt(input_text4, chushi))
        thread = threading.Thread(target=getgptinfo, args=(input_text4, chushi,))
        thread.start()
        return response

if __name__ == '__main__':
    baseproxy = AsyncMitmProxy(server_addr=('127.0.0.1',8899),https=True)
    print("中间人代理启动成功，gpt3.5 将辅助您进行测试")
    baseproxy.register(LewisInterceptor)
